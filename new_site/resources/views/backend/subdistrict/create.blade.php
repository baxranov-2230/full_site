...
{{--                          action="{{route('store.subcategory')}}"--}}
                        ...
                        <div class="form-group">
                            <label for="exampleInputEmail1">SubDistrict Uzbek</label>
                            <input type="text" class="form-control" name="subdistrict_uz" placeholder="SubDistrict Uzbek">
                            @error('subdistrict_uz')
                            <span class="text-danger">{{$message}}</span>
                            @enderror
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" name="district_id" id="exampleFormControlSelect2">
                                ...
                                @foreach($district as $row)
                                    <option value="{{$row->id}}">{{$row->district_en}} | {{$row->district_uz}}</option>
                                @endforeach
                            </select>
                        </div>
